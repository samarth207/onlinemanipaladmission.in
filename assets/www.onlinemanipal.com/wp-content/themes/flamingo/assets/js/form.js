  ...
  function openPdfPopup(pdfLink) {
    ...
    function addOverlayLinkForPage(pageNum) {
      ...
      if (university_url == "MUJ") {
        var login_url = "https://login.muj.onlinemanipal.com/";
      } else if (university_url == "SMU") {
        var login_url = "https://login.smu.onlinemanipal.com/";
      } else {
        var login_url = "https://login.mahe.onlinemanipal.com/";
      }
      ...
      if ($(formSelector).valid()) {
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            } else if (response.status === 2 && response.prospectId) {
              const prospectId = response.prospectId;
              localStorage.setItem('formSubmitted', "Yes");
              localStorage.setItem('prospectId', prospectId);
              localStorage.setItem('leadID', prospectId);
              localStorage.setItem('mobileUpdate', "Required");
              sendOTP(formSelector);
              formSelector !== downloadForm && LSQSubmission(formSelector, e);
            } else {
              errorDisplay(formSelector, response)
